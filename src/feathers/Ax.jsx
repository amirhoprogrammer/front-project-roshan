...
const handleSubmit1 = async (e) => {
  ...
  try {
    ...
    formData.append("file", file); // فایل به جای media_urls
...
>
  {showForm ? (
    <div className="p-8">
      <div
        className="rounded-full flex flex-row-reverse w-12 mx-24 "
        style={{ border: "1px solid #118AD3" }}
      >
        <form onSubmit={handleSubmit1} className="flex flex-col w-12">
          <input
            type="file"
            accept="audio/*,video/*"
      ...
      <p className="mt-2" style={{ textAlign: "center", color: "#626262" }}>
        برای بارگذاری فایل گفتاری (صوتی/تصویری)، دکمه را فشار دهید متن پیاده شده آن، در اینجا ظاهر
        می شود
      </p>
      ...
      <div className="flex flex-col p-4">
        <div className="flex border-b-1 flex-row border-b-black my-0 h-10 p-0">
          ...
          <div
            ...
            onClick={() =>
              activeButton === 0 ? copyToClipboard(transcription) : copyToClipboard(getTimedText())
            }
          >
            <svg
              width="16"
