...
// تابع برای گرفتن حجم فایل
const getFileSize = async (url) => {
  try {
    const response = await axios.head(url);
    const sizeInBytes = response.headers["content-length"];
    if (sizeInBytes) {
      const sizeInMB = (sizeInBytes / (1024 * 1024)).toFixed(2); // تبدیل به مگابایت
      return `${sizeInMB} مگابایت`;
    }
    return "نامشخص";
  } catch (err) {
    console.error("خطا در گرفتن حجم فایل: ", err);
    return "نامشخص";
  }
};
...
function Archive() {
  ...
  const getRequestType = (request) => {
    if (request.url && request.url.startsWith("http://harf.roshan-ai.ir/media/files/")) {
      return "ضبط صدا";
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        ...
        // گرفتن حجم فایل برای هر درخواست
        const sizes = {};
        for (const request of requestsData) {
          if (request.url) {
            sizes[request.url] = await getFileSize(request.url);
          }
        }
        setFileSizes(sizes);
        ...
        <div className="my-2">
          {requests.map((request, index) => {
            ...
            return (
              ...
              >
                <div className="flex mb-2" onClick={() => setActiveDiv(1)}>
                  {requestType === "ضبط صدا" && (
                  ...
                  >
                    ...
                    <Tooltip
                      id={`tooltip-${index}`}
                      style={{ backgroundColor: "#FFFFFF", color: "#000000B2" }}
                    />
                  </div>
                  <div
                    className="mx-2"
